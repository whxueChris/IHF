...
def parse_args():
    ...
    parser.add_argument("--mode", type=str, default="TDec",
                        choices=["TDec"],
...
class PureTransformerDecoder(nn.Module):
    def __init__(self, input_size, proj_dim, nhead, num_layers, ff_dim, decoder_layers, mode="standard"):
        super(PureTransformerDecoder, self).__init__()
        self.mode = mode
    ...
    def forward(self, pre_x, tgt):
        x = self.proj_nonlinear(self.proj_linear(pre_x))
        fused_feature = self.transformer_encoder(x)
...
def train_and_evaluate_models(all_file_path: Union[list, str] = None, window_size_range: tuple = (15, 15),
    ...
    model = PureTransformerDecoder(input_size, proj_dim, nhead, num_encoder_layers, ff_dim, decoder_layers, mode=mode)
    ...
    if not args.evaluate_only:
        for epoch in range(num_epochs):
            ...
            for inputs, targets, center_points, _ in tqdm(train_loader, desc=f"Epoch {epoch + 1} training"):
                ...
                tgt = torch.randn(bs, targets.shape[1], 2, device=device)  # Initial target for autoregressive
            ...
            # Validation
            val_loss = pure_evaluate(model, val_loader, criterion, device, mode)
        ...
        model.load_state_dict(
            torch.load(f'best_model_{save_mode}_ws{window_size_range[0]}_ws{window_size_range[1]}.pth'))
        test_loss = pure_evaluate(model, test_loader, criterion, device, mode)
        ...
        for val_window_size_length in range(val_window_size_range[0], val_window_size_range[1] + 1):
            test_data_dict, test_target_dict = process_dataset_and_split(list_csv, (
            val_window_size_length, val_window_size_length), test_index)
            test_dataset = CustomDataset(test_data_dict, test_target_dict, mode='test')
            test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
            test_loss, per_sample_data = pure_evaluate(model, test_loader, criterion, device, mode,
                                                       recursive_evaluate=True, save_per_sample=True)
            # Add window size to each sample's data
...
def pure_evaluate(model, data_loader, criterion, device, mode, recursive_evaluate=False, save_per_sample=False):
    ...
    with torch.no_grad():
        for inputs, targets, center_points, _ in tqdm(data_loader, desc="Evaluating"):
            ...
            # Calculate per-sample losses if requested
            if save_per_sample:
